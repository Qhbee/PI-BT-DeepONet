...
**用法**（在项目根目录）::
    ...
    # MC 可信带可用 ``--no_ci`` 关闭；``--eval_mc_samples`` 默认读 config
...
def _predict_mean_and_ci(
    ...
    *,
    ...
    device: str,
    compute_ci: bool,
    ci_outer: tuple[float, float] = (0.025, 0.975),
    ci_inner: tuple[float, float] = (0.10, 0.90),
    ...
    Returns:
        mean: (n_points,)
        ci: None if not bayesian or not compute_ci; if set, ((lo95, hi95), (lo80, hi80))
            using equal-tailed MC quantiles over sampled forward passes (epistemic).
    """
    model.eval()
    with torch.no_grad():
        u = u.to(device)
...
def run_plot(
    *,
    ...
    if eval_mc_samples is None:
        eval_mc_samples = int(cfg.get("eval_mc_samples", DEFAULT_EVAL_MC_SAMPLES))
    ...
    for k, (f_tex, s_tex, f_fn, s_fn) in enumerate(cases):
        ...
        ax.plot(x_query, s_pred, color="blue", lw=1.2, ls="--", label=r"pred $\hat{s}(x)$ mean")
...
def main() -> None:
    ...
    p.add_argument(
        "--no_ci",
        action="store_true",
        help="贝叶斯时不绘制 80%%/95%% MC 可信带（仍用 MC 均值作预测曲线）",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help=f"贝叶斯 MC 采样数（默认: 读 config 或 {DEFAULT_EVAL_MC_SAMPLES}）",
    )
